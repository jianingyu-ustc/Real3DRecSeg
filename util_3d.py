...
def transform_points(matrix, points):
    ...
    p = np.transpose(p)     # shape: (237360, 4)
    p[:,:3] /= p[:,3,None]
    return p[:,:3]  # (237360, 3), [-2.84436026  0.1550739   2.42560415] ...
...
def read_mesh_vertices(filename, label):
    assert os.path.isfile(filename)
    with open(filename, 'rb') as f:
        plydata = PlyData.read(f)
        num_verts = plydata['vertex'].count
        if label:
            vertices = np.zeros(shape=[num_verts, 4], dtype=int)
        else:
            vertices = np.zeros(shape=[num_verts, 3], dtype=int)
        
        for i in range(len(plydata['vertex'].data['x'])):
            plydata['vertex'].data['x'][i] = int(round(10 * plydata['vertex'].data['x'][i]) + 100)
        vertices[:,0] = plydata['vertex'].data['x']   # col 0
        
        for i in range(len(plydata['vertex'].data['y'])):
            plydata['vertex'].data['y'][i] = int(round(10 * plydata['vertex'].data['y'][i]) + 100)
        vertices[:,1] = plydata['vertex'].data['y']   # col 1
        
        for i in range(len(plydata['vertex'].data['z'])):
            plydata['vertex'].data['z'][i] = int(round(10 * plydata['vertex'].data['z'][i]) + 100)
